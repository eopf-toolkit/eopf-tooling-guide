...
As mentioned previously, Zarr data arrays are often packed or compressed in order to limit space, and may need to be scaled or offset to their actual physical units or meaningful values. 

We use the helper function previously created to get the scale and offset for this data.

```{r owi-scale-factor-offset}
...
Since the `scale` values are all 1, and the `offset` are all 0, none of this data need to be scaled or offset.
...
For Sentinel-2, we will calculate the Normalized Difference Vegetation Index (NDVI).
...
This example shows how to access the 60-metre resolution quicklook of a Sentinel-2 mission, explored in more detail [above](#quicklook-composite-image). We set up `zarr_start` and `zarr_end` to time the data retrieval and visualisation process, for comparison to the SAFE process later on.
...
`CDSE_USERNAME` and `CDSE_PASSWORD` (using e.g. `usethis::edit_r_environ()` to set these).
...
The EOPF Zarr example took `r zarr_time` seconds, while the SAFE example took `r safe_time` minutes---a difference of nearly 6 minutes.

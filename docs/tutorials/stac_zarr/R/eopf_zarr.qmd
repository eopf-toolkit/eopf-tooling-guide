...
```{r dl-safe}
item <- stac("https://catalogue.dataspace.copernicus.eu/stac/") %>%
  collections(collection_id = "SENTINEL-2") |>
  items(feature_id = "S2A_MSIL2A_20250605T104701_N0511_R051_T35WMU_20250605T125416.SAFE") |>
  get_request()
...
asset_url <- stringr::str_replace(asset_url, "\\$value", glue::glue("{item_id}.zip"))
...
# asset$assets$PRODUCT$href <- asset_url


httr2::request(asset$assets$PRODUCT$href) |>
...
token <- httr2::oauth_flow_client_credentials(
  client =
    httr2::oauth_client(id = Sys.getenv("CDSE_USERNAME"), "https://identity.dataspace.copernicus.eu/auth/realms/CDSE/protocol/openid-connect/token",
  secret = Sys.getenv("CDSE_PASSWORD")
  )
)

GET(asset$assets$PRODUCT$href, 
    )

assets_download(
  asset,
  config = add_headers(Authorization = paste("Bearer", token$access_token, sep = " ")))
```

...
We will use the `rstac` package (for accessing the STAC catalog) and the `purrr` package (for data manipulation) in this tutorial. You can install them directly from CRAN:
...
The first step of accessing Zarr data is to find the correct asset within the EOPF Sample Service STAC catalog. The [first tutorial](TODO) goes into detail on this, so we recommend reviewing it if you have not already.
